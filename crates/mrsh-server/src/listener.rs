...
async fn bind_reusable(addr: SocketAddr) -> io::Result<TcpListener> {
    ...
    bind_reusable_inner(addr, domain, false)
}

fn bind_reusable_inner(
    addr: SocketAddr,
    ...
    if dual_stack {
        // IPV6_V6ONLY = false → accept both IPv4 (mapped) and IPv6
...
async fn handle_mux_connection(
    ...
    match buf[0] {
        ...
        0x53 => {
            // SSH version string starts with 'S' (from "SSH-2.0-...")
            let peek_stream = PeekStream::new(buf[0], stream);
            info!("SSH connection from {}", peer);
            if let Err(e) = crate::ssh::handle_ssh_connection(peek_stream, ctx).await {
                warn!("SSH connection from {} error: {}", peer, e);
            }
        }
        other => {
