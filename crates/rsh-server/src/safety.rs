//! Exec safety guard — blocks commands that would kill/stop this rsh process.
//!
//! AI agents frequently use rsh for remote management. Without server-side
...
pub enum SafetyVerdict {
    ...
    Allow,
    /// Command would kill/stop/remove rsh itself.
    Block { reason: String },
}

/// Check if a command would be self-destructive for the rsh server process.
///
...
pub fn check_exec(command: &str) -> SafetyVerdict {
    ...
    if normalized.contains("taskkill") && normalized.contains("rsh") {
        return block("taskkill would kill the rsh process serving this connection. \
    ...
    if (normalized.contains("stop-service") || normalized.contains("net stop"))
        && (normalized.contains("rsh") || normalized.contains("remote-shell")
            || normalized.contains("remote shell"))
    {
        return block("Stopping the rsh service would cut off this connection. \
    ...
    if normalized.contains("stop-process") && normalized.contains("rsh") {
        return block("Stop-Process would kill the rsh process serving this connection. \
    ...
    // Pattern 5: sc delete targeting rsh service
    if normalized.contains("sc") && normalized.contains("delete")
        && (normalized.contains("rsh") || normalized.contains("remote-shell"))
    {
        return block("Deleting the rsh service registration would prevent restart. \
...
mod tests {
    ...
    fn blocks_stop_service() {
        ...
        assert!(is_blocked("net stop remote-shell"));
        assert!(is_blocked("Stop-Service 'remote-shell'"));
    }

    #[test]
    fn blocks_stop_process() {
        assert!(is_blocked("Stop-Process -Name rsh"));
        assert!(is_blocked("Stop-Process -Name rsh -Force"));
        assert!(is_blocked("Get-Process rsh | Stop-Process"));
    }

    #[test]
    fn blocks_delete_binary() {
        assert!(is_blocked("Remove-Item C:\\ProgramData\\remote-shell\\rsh.exe"));
        assert!(is_blocked("del C:\\ProgramData\\remote-shell\\rsh.exe"));
    }

    #[test]
    fn blocks_sc_delete() {
        assert!(is_blocked("sc delete rsh"));
        assert!(is_blocked("sc delete remote-shell"));
    }
    ...
    fn allows_safe_commands() {
        ...
        assert!(is_allowed("dir C:\\ProgramData\\remote-shell\\"));
    ...
    #[test]
    fn allows_rsh_client_commands() {
        // Running rsh as a client command is fine
        assert!(is_allowed("rsh -h other-host ping"));
        assert!(is_allowed("C:\\ProgramData\\remote-shell\\rsh.exe -h 192.168.1.1 exec hostname"));
    }
}
