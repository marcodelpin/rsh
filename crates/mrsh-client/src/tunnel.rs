//! Client-side TCP tunnel — `ssh -L` equivalent over the mrsh protocol.
//!
//! Binds a local TCP listener and forwards each accepted connection through
//! the mrsh server's "connect" request type, providing transparent TCP tunneling.
//!
//! # Protocol
//!
//! 1. Client binds a local TCP port (e.g., `127.0.0.1:5432`)
//! 2. For each incoming local connection:
//!    a. Send a "connect" request to the mrsh server with the remote target
//!    b. Server connects to the target and sends back success
//!    c. Bidirectional relay: local TCP ↔ mrsh wire ↔ remote target
//!
//! # Wire format
//!
...
//! mrsh -h server tunnel 127.0.0.1:5432 remote_db:5432
//! ```
...
/// Run a local TCP tunnel that forwards connections through the mrsh server.
///
/// This function:
/// 1. Binds a local TCP listener on `local_bind`
/// 2. For each accepted connection, establishes a tunnel through the mrsh server
/// 3. Relays data bidirectionally until either side closes
///
/// The mrsh connection must already be authenticated. This function sends a
/// "connect" request for each incoming local connection using a fresh rsh
/// connection (since "connect" hijacks the connection).
///
/// # Arguments
/// * `stream` - Authenticated mrsh stream (will be hijacked for the first connection)
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    // Send "connect" request to mrsh server
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    loop {
        tokio::select! {
            ...
            // mrsh → Local: receive wire frame, write to local TCP
            result = wire::recv_message(rsh_stream) => {
                match result {
                    ...
                    Err(_) => {
                        debug!("tunnel: mrsh connection closed");
                        local_write.shutdown().await.ok();
                        break;
                    }
                }
...
///
/// For each connection, establishes a new mrsh session with `connect_fn`,
...
/// * `connect_fn` - Factory function that creates a new authenticated mrsh connection
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    loop {
        ...
        match connect_fn().await {
            ...
            Err(e) => {
                warn!("multi-tunnel: failed to connect to mrsh server: {}", e);
                drop(local_stream);
            }
        }
    }
...
mod tests {
    ...
    async fn relay_tunnel_echo_roundtrip() {
        ...
        // Simulate the mrsh server side (reads connect request, sends ack, relays)
        let (mut mrsh_client, mut mrsh_server) = tokio::io::duplex(8192);

        let target = echo_addr.to_string();
        let server_handle = tokio::spawn(async move {
            // Read connect request from client
            let req: protocol::Request = wire::recv_json(&mut mrsh_server).await.unwrap();
            ...
            wire::send_json(&mut mrsh_server, &ack).await.unwrap();
            ...
            loop {
                tokio::select! {
                    result = wire::recv_message(&mut mrsh_server) => {
                        match result {
                    ...
                    result = target_read.read(&mut target_buf) => {
                        match result {
                            Ok(0) => {
                                wire::send_message(&mut mrsh_server, &[]).await.ok();
                                break;
                            }
                            Ok(n) => {
                                wire::send_message(&mut mrsh_server, &target_buf[..n]).await.unwrap();
                            }
                            Err(_) => break,
                        }
                    }
        ...
        let tunnel_handle = tokio::spawn(async move {
            ...
            relay_tunnel(&mut mrsh_client, local_stream).await.unwrap();
        });
    ...
    async fn relay_tunnel_server_closes_first() {
        // Simulate mrsh server that connects and immediately closes
        let (mut mrsh_client, mut mrsh_server) = tokio::io::duplex(8192);

        let server_handle = tokio::spawn(async move {
            // Read connect request
            let _: protocol::Request = wire::recv_json(&mut mrsh_server).await.unwrap();
            ...
            wire::send_message(&mut mrsh_server, &[]).await.unwrap();
        });
    ...
    /// Helper: spawn a mock mrsh server that handles the "connect" request
    /// and then echoes data back through wire-protocol framing.
    fn spawn_echo_mrsh_server(
        mut mrsh_server: tokio::io::DuplexStream,
        ...
        tokio::spawn(async move {
            ...
            loop {
                match wire::recv_message(&mut mrsh_server).await {
                    Ok(data) if data.is_empty() => {
                        wire::send_message(&mut mrsh_server, &[]).await.ok();
                        break;
                    }
                    Ok(data) => {
                        wire::send_message(&mut mrsh_server, &data).await.unwrap();
                    }
                    Err(_) => break,
                }
            }
    ...
    async fn run_tunnel_echo_roundtrip() {
        let (mut mrsh_client, mrsh_server) = tokio::io::duplex(8192);
        let server_handle = spawn_echo_mrsh_server(mrsh_server);

        let local_addr = free_port().await;

        // run_tunnel binds the port, accepts one connection, sends connect, relays
        let tunnel_handle = tokio::spawn(async move {
            run_tunnel(&mut mrsh_client, &local_addr.to_string(), "echo:1234").await
        });
    ...
    async fn run_tunnel_server_rejects_connect() {
        ...
        let tunnel_handle = tokio::spawn(async move {
            run_tunnel(&mut mrsh_client, &local_addr.to_string(), "bad:9999").await
        });
    ...
    async fn run_tunnel_bind_failure() {
        ...
        let (mut mrsh_client, _mrsh_server) = tokio::io::duplex(8192);
        let result = run_tunnel(&mut mrsh_client, &addr.to_string(), "host:80").await;
        assert!(result.is_err());
    ...
    async fn run_multi_tunnel_handles_two_connections() {
        ...
        let connect_fn = move || {
            ...
            async move {
                ...
                spawn_echo_mrsh_server(server_end);
                Ok(client_end)
            }
        };
