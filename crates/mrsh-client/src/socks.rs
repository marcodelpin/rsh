//! SOCKS5 proxy — `ssh -D` equivalent over the mrsh protocol.
//!
...
//! 3. Opens a new mrsh connection, sends "connect" to the mrsh server
//! 4. Relays traffic bidirectionally
//!
//! # Example
//!
//! ```text
//! mrsh -h server -D 1080
...
/// Run a SOCKS5 proxy that tunnels connections through the mrsh server.
///
/// For each SOCKS5 CONNECT request, establishes a new mrsh connection
/// via `connect_fn`, sends a "connect" request to the mrsh server, and
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    loop {
        ...
        match connect_fn().await {
            ...
            Err(e) => {
                warn!("SOCKS5: failed to connect to mrsh server: {}", e);
                drop(client_stream);
            }
        }
    }
...
/// request to the mrsh server, and relays traffic.
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    // Receive acknowledgment from mrsh server
...
/// Relay traffic between a SOCKS5 client and the mrsh wire protocol.
///
    ...
    C: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    loop {
        tokio::select! {
            ...
            // mrsh → Client: receive wire frame, write to SOCKS5 client
            result = wire::recv_message(rsh_stream) => {
                match result {
                    ...
                    Err(_) => {
                        debug!("SOCKS5: mrsh connection closed");
                        client_write.shutdown().await.ok();
                        break;
                    }
                }
...
mod tests {
    ...
    async fn socks5_handshake_ipv4() {
        ...
        let (mut mrsh_client, mut mrsh_server) = tokio::io::duplex(4096);

        let handler = tokio::spawn(async move {
            // Simulate mrsh server: read connect request, send success ack, then EOF
    ...
    async fn socks5_server_rejects() {
        ...
        let socks_handler = tokio::spawn(async move {
            let result = handle_socks5_client(&mut server_end, &mut mrsh_client).await;
