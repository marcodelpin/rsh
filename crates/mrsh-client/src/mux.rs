...
//! mrsh invocations reuse it through the socket, avoiding TLS+auth
//! overhead on every command.
//!
//! Usage:
//!   mrsh -h host -M          Start master (foreground, Ctrl+C to stop)
//!   mrsh -h host exec cmd    Auto-uses master if running
//!   mrsh -h host --no-mux    Skip multiplexing, always open new connection
//!   mrsh -h host --mux-stop  Stop running master for this host
